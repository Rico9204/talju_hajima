...
interface ProjectContextValue {
  ...
  addFile: (name: string, content: string, folderId: number | null, note?: string) => Promise<void>;
  addFileVersion: (fileId: number, content: string, note?: string) => Promise<void>;
...
export function ProjectProvider({ children }: { children: ReactNode }) {
  ...
  async function accountIdentity(): Promise<{ name: string; avatar: string }> {
    const name = session?.user.name?.trim() || session?.user.email?.split("@")[0] || "사용자";
  ...
  async function addFile(name: string, content: string, folderId: number | null, note?: string) {
    if (!projectId || !currentMember) return;
    await dataRepository.createFile(projectId, { name, folderId, content, note }, currentMember.name, currentMember.avatar);
    await refreshFiles();
  }

  async function addFileVersion(fileId: number, content: string, note?: string) {
    if (!currentMember) return;
    await dataRepository.addFileVersion(fileId, currentMember.name, note, content);
    await refreshFiles();
  }
