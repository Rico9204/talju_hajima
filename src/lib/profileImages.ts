export const PROFILE_IMAGE_MIME_TYPES = ["image/jpeg", "image/png", "image/webp", "image/avif", "image/gif"] as const;
...
export async function validateProfileImage(file: File): Promise<ProfileImageMimeType> {
  if (!PROFILE_IMAGE_MIME_TYPES.includes(file.type as ProfileImageMimeType)) {
    throw new Error("프로필 이미지는 JPG, PNG, WebP, AVIF, GIF 형식만 사용할 수 있습니다.");
  }
