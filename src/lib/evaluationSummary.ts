...
export function summarizeEvaluations(members: Pick<Member, "score" | "evalCount" | "criteriaScores">[], projectCount: number) {
  ...
  return { projectCount, count, score: count ? members.reduce((sum, m) => sum + m.score * m.evalCount, 0) / count : null, criteria };
}
