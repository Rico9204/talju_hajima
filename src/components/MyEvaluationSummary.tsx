    ...
    {error ? <p role="alert" className="text-sm">평가 요약을 불러오지 못했습니다. <button onClick={() => setRetry((n) => n + 1)}>다시 시도</button></p>
      ...
      : <>
        <div className="grid grid-cols-2 gap-4">
          ...
          <div><div className="text-xs">프로젝트 참여 횟수</div><strong className="text-xl">{completedOnly ? projects.filter((p) => p.status === "done").length : result.projectCount}회</strong><p className="text-xs">{completedOnly ? "종료된 프로젝트 기준" : "진행 중·종료 포함"}</p></div>
        </div>
