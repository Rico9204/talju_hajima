    ...
    {error ? <p role="alert" className="text-sm">평가 요약을 불러오지 못했습니다. <button onClick={() => setRetry((n) => n + 1)}>다시 시도</button></p>
      ...
      : <>
        <div className={`grid grid-cols-2 ${chart ? "gap-2" : "gap-4"}`}>
          <div><div className={labelSize}>내 평가 평균</div><strong className={valueSize} style={{ color: "var(--primary)" }}>{result.score === null ? "공개 대기" : result.score.toFixed(1) + " / 10"}</strong><p className={labelSize}>받은 최종 평가 {result.count}건 기준</p></div>
        ...
        <p className={`${labelSize} ${chart ? "mt-2" : "mt-3"}`} style={{ color: "var(--muted-foreground)" }}>전체 참여 프로젝트의 최종 평가 평균입니다. 중간 평가는 포함하지 않습니다. 프로젝트마다 동료 2명 이상이 모두 제출해야 평균이 공개됩니다.</p>
