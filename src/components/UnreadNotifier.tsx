...
export default function UnreadNotifier() {
  ...
  useEffect(() => {
    ...
    // 채팅 기록을 처음 불러오는 동안 늘어난 안 읽음 수·멘션은 쌓여 있던 것이므로 알리지 않는다.
    const chatReady = chatHistoryLoaded && prev.chatLoaded;
    const addedMentions = chatReady ? unreadMentions.filter((m) => !prev.mentionIds.has(m.messageId)) : [];
    const addedChat = chatReady ? Math.max(0, chatUnreadTotal - prev.chat) : 0;
