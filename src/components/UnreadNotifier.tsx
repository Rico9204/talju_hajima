...
export default function UnreadNotifier() {
  ...
  const previous = useRef<{ taskIds: Set<number>; scheduleIds: Set<number>; fileIds: Set<number>; chat: number } | null>(null);
  ...
  useEffect(() => {
    ...
    previous.current = { taskIds, scheduleIds, fileIds, chat: chatUnreadTotal };
    ...
    const addedChat = Math.max(0, chatUnreadTotal - prev.chat);
  ...
  }, [newTasks, newScheduleEvents, newFiles, chatUnreadTotal]);
