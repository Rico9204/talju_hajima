...
// Lets an admin manage the roster of ANY project (not just ones they're a
// member of) from AdminPanel — matches gwanhan.md's "팀원 초대/제외: 관리자는
// 모든 프로젝트" row. getTeam()/kickMember() already RLS-allow this for
// admins; this just gives the UI a way to reach a project the admin never
// joined (TeamView only ever shows the signed-in account's own projects).
export default function AdminProjectMembers({ project, onClose }: { project: Project; onClose: () => void }) {
  ...
  async function refresh() {
    ...
    setError(null);
    try {
      setTeam(await dataRepository.getTeam(project.id));
    } catch (err) {
