...
export default function WorkspaceComments({ file }: { file: WorkspaceFile }) {
  ...
  return <div className="flex flex-col gap-3">
    ...
    {!file.comments.length && <p className="text-xs text-center py-3" style={{ color: "var(--muted-foreground)" }}>아직 댓글이 없어요. 첫 코멘트를 남겨보세요.</p>}
    ...
    {!locked && <div>
      ...
      <form className="flex gap-2 mt-1" onSubmit={(e) => { e.preventDefault(); if (draft.trim()) void run(async () => { await addFileComment(file.id, draft); setDraft(""); setPicker(null); }); }}>
