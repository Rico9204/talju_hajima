...
export default function FileVersionPanel({ file, searchQuery = "", onViewingVersionChange, onQuickEdit, editorNames = [] }: { file: WorkspaceFile; searchQuery?: string; onViewingVersionChange?: (versionId: number | null) => void; onQuickEdit?: (mode: "main" | "pin", version?: FileVersion) => void; editorNames?: string[] }) {
  ...
  async function openVersion(v: FileVersion, download: boolean) {
    ...
    } else if (isRichDocName(name) || isSlidesName(name)) {
      setPreview({ text: v.searchText || "(내용 없음)", kind: "text", name });
  ...
  return <div>
    ...
    {!locked && onQuickEdit && currentVersion?.storagePath && (isEditableTextFile(currentVersion.originalName ?? file.name, currentVersion.byteSize) || isRichDocName(currentVersion.originalName ?? file.name) || isSlidesName(currentVersion.originalName ?? file.name)) && (
