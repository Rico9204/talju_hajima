...
begin
  ...
  if p_phase='midterm' and exists(select 1 from jsonb_array_elements(p_entries) e cross join unnest(array['role','deadline','communication','collaboration','quality']) k group by k having sum((e->>k)::integer)<>expected*5) then raise exception '각 항목의 총점은 동료 수 × 5점이어야 합니다.'; end if;
