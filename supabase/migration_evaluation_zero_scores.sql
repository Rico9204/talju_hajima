-- Apply after migration_evaluation_prototype.sql; preserves existing policies and phase gating.
begin;
alter table public.peer_evaluations drop constraint if exists peer_evaluations_role_check;
alter table public.peer_evaluations add constraint peer_evaluations_role_check check (role between 0 and 10);
alter table public.peer_evaluations drop constraint if exists peer_evaluations_deadline_check;
alter table public.peer_evaluations add constraint peer_evaluations_deadline_check check (deadline between 0 and 10);
alter table public.peer_evaluations drop constraint if exists peer_evaluations_communication_check;
alter table public.peer_evaluations add constraint peer_evaluations_communication_check check (communication between 0 and 10);
alter table public.peer_evaluations drop constraint if exists peer_evaluations_collaboration_check;
alter table public.peer_evaluations add constraint peer_evaluations_collaboration_check check (collaboration between 0 and 10);
alter table public.peer_evaluations drop constraint if exists peer_evaluations_quality_check;
alter table public.peer_evaluations add constraint peer_evaluations_quality_check check (quality between 0 and 10);
